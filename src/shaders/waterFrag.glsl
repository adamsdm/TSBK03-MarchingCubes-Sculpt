...
void main() {
    vec3 waterColor = vec3(0.0, 0.2, 0.8);
    vec3 white = vec3(1.0);
    ...
    */
    ...
    vec3 grad = vec3(0.0);
    float waterNoise = snoise(1.0 * vec3(pos.x, pos.y*4.0, pos.z) * 0.5 + vec3(0.0, 1.0, 0.0) * time, grad);

    // Perturb normal
    vec3 nNormal = vNormal;
    vec3 perturbation = grad - dot(grad, vNormal) * vNormal;
    nNormal -= waterNoise * 0.2*perturbation;
    ...
    float spec = pow(max(dot(V,R), 0.0), 1.0);
